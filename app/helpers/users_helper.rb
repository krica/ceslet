module UsersHelper
  ...
  def user_destroy_link(user)
    ...
    Offer.all.each do |offer|
      if offer.responsible_person_bussiness == user.id || offer.basis_processer_id == user.id || offer.classification_approval_id == user.id || offer.basisi_approval_id == user.id
        count = count + 1
      end
    end
    if count == 0
      return link_to_with_icon('trash',t('Destroy') , user, :confirm => t('Are you sure'), :method => :delete, :class => "fg-btn-small to-btn")
    else
